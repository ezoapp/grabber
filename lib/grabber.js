...
function Grabber(data, options, logger, originUrl, socket) {
  this.$ = null;
  this.data = data;
  this.baseUrl = '';
  this.tasklist = [];
...
Grabber.prototype = (function () {
  ...
  var RES_TAG = {
    ...
    EXCLUDE_LINK = ['http://jqmdesigner.appspot.com/components/platform/platform.js'],
    ...
    _.each(RES_TAG, function (tags, attr) {
      _.each(tags, function (tag) {
        _.each($(tag), function (ele, idx) {
          ...
          if (url && EXCLUDE_LINK.indexOf(url) < 0) {
            if (isValidUrl(url)) {
